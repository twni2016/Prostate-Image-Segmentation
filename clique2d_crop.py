...
if __name__ == '__main__':
	...
	parser.add_argument('-e', '--epoch', type=int, default=60, help='number of epochs to train (default: 50)')
...
class CliqueNet(nn.Module):
	def __init__(self, input_channels, list_channels, list_layer_num):
		...
		self.fir_trans = nn.Conv2d(1, input_channels, kernel_size=7, stride=2, padding=3, bias=False) # conv 7*7 /2 = 224*224
		self.fir_bn = nn.BatchNorm2d(input_channels)
		# self.fir_pool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1) # /2 = 144*144
...
# 				nn.ELU(inplace=True)
# 			)
# 		self.down0 = DownTransition(inchan=8,layer=2,dilation_=2)  # 16*224^2
# 		self.down1 = DownTransition(inchan=16,layer=2,dilation_=2) # 32*144^2
# 		self.down2 = DownTransition(inchan=32,layer=2,dilation_=2) # 64*64^2
# 		self.down3 = DownTransition(inchan=64,layer=2,dilation_=4) # 144*32^2
		

# 		self.up3 = UpTransition(inchan=144,layer=2) # 64*64^2
# 		self.up2 = UpTransition(inchan=64,layer=2) # 32*144^2
# 		self.up1 = UpTransition(inchan=32,layer=2) # 16*224^2
...
if __name__ == '__main__':
	...
	model = CliqueNet(input_channels=32, list_channels=[20, 20, 20], list_layer_num=[5, 5, 5])
	...
	scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', patience=6, threshold=0.03, threshold_mode='abs')
	...
	for e in range(epoch):
		...
		for index,(image,target) in enumerate(trainloader):
			...
			output = model(image_center) # (NHW,2)
		...
		print ("Epoch[%d/%d], Train Dice Coef Truly: %.4f" %(e+1, epoch, true_loss/len(trainloader)))
		...
		print ("Epoch[%d/%d], Valid Dice Coef Truly: %.4f" %(e+1, epoch, true_loss/len(testloader)))
		scheduler.step(true_loss/len(testloader))
	...
	# torch.save(model.state_dict(),'model'+str(datetime.now())[5:16]+'.pkl')
