...
image_set = np.load('data/PROS_Train/image2d.npy')
label_set = np.load('data/PROS_Train/label2d.npy')

# label_set = 1*(label_set.sum((1,2))>0)
# print(label_set.max())

train_image_set = np.zeros((1185*7,512,512))
train_label_set = np.zeros((1185*7,512,512))
...
for i in range(0,1185):
	...
	train_image_set[cnt+4] = np.roll(train_image_set[i],shift=4,axis=1) # horizontal translation
	train_label_set[cnt+4] = train_label_set[i]
	train_image_set[cnt+5] = np.roll(train_image_set[i],shift=-4,axis=1) # horizontal translation
	train_label_set[cnt+5] = train_label_set[i]
